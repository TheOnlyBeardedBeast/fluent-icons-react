import * as React from 'react';
import IconProps from '../IconProps';

export const SelectObjectFilled: React.FC<IconProps> = ({ size, color }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 20 20"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M6 4.5C6 5.32843 5.32843 6 4.5 6C3.67157 6 3 5.32843 3 4.5C3 3.67157 3.67157 3 4.5 3C5.32843 3 6 3.67157 6 4.5Z"
      fill={color}
    />
    <path
      d="M17 4.5C17 5.32843 16.3284 6 15.5 6C14.6716 6 14 5.32843 14 4.5C14 3.67157 14.6716 3 15.5 3C16.3284 3 17 3.67157 17 4.5Z"
      fill={color}
    />
    <path
      d="M6 15.5C6 16.3284 5.32843 17 4.5 17C3.67157 17 3 16.3284 3 15.5C3 14.6716 3.67157 14 4.5 14C5.32843 14 6 14.6716 6 15.5Z"
      fill={color}
    />
    <path
      d="M17 15.5C17 16.3284 16.3284 17 15.5 17C14.6716 17 14 16.3284 14 15.5C14 14.6716 14.6716 14 15.5 14C16.3284 14 17 14.6716 17 15.5Z"
      fill={color}
    />
    <path
      d="M7 4.75C7 4.33579 7.33579 4 7.75 4H12.25C12.6642 4 13 4.33579 13 4.75C13 5.16421 12.6642 5.5 12.25 5.5H7.75C7.33579 5.5 7 5.16421 7 4.75Z"
      fill={color}
    />
    <path
      d="M7 15.25C7 14.8358 7.33579 14.5 7.75 14.5H12.25C12.6642 14.5 13 14.8358 13 15.25C13 15.6642 12.6642 16 12.25 16H7.75C7.33579 16 7 15.6642 7 15.25Z"
      fill={color}
    />
    <path
      d="M4 7.75C4 7.33579 4.33579 7 4.75 7C5.16421 7 5.5 7.33579 5.5 7.75V12.25C5.5 12.6642 5.16421 13 4.75 13C4.33579 13 4 12.6642 4 12.25V7.75Z"
      fill={color}
    />
    <path
      d="M14.5 7.75C14.5 7.33579 14.8358 7 15.25 7C15.6642 7 16 7.33579 16 7.75V12.25C16 12.6642 15.6642 13 15.25 13C14.8358 13 14.5 12.6642 14.5 12.25V7.75Z"
      fill={color}
    />
  </svg>
);
